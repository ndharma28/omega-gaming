...
function GlyphRow({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-center gap-3 text-red-900/60 text-xs tracking-[0.4em] select-none my-1">
      {children}
    </div>
  );
}
...
function Sigil() {
  return (
    <svg width="48" height="48" viewBox="0 0 48 48" fill="none" className="text-red-600/40">
...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="bg-black/40 border border-red-900/20 rounded-2xl p-5 space-y-1 backdrop-blur-sm">
...
export default function ChronicleePage() {
  ...
  return (
    <main ref={containerRef} className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      {/* Radial glow */}
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(ellipse_at_50%_0%,rgba(127,29,29,0.25)_0%,transparent_70%)]" />
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-16 space-y-16">
        ...
        <div className="text-center space-y-6">
          ...
          <div className="space-y-1">
            <p className="text-[10px] tracking-[0.5em] text-red-700 uppercase font-bold">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-[11px] font-bold border transition-all ${
                  filterRank === rank
                    ...
                    : "bg-black/40 border-red-900/20 text-slate-600 hover:text-slate-400"
                }`}
              >
                {rank}
              </button>
          ...
          <button
            ...
            className="text-[11px] text-slate-600 hover:text-slate-300 font-bold flex items-center gap-1 transition-colors"
          >
        ...
        {/* ── Table ──────────────────────────────────────────────────────────── */}
        <div className="rounded-2xl border border-red-900/20 overflow-hidden backdrop-blur-sm">
          {/* Table header */}
          <div className="grid grid-cols-[3rem_1fr_1fr_1fr_6rem] gap-4 px-6 py-3 bg-red-950/30 border-b border-red-900/20">
            {["#", "Address", "Prize", "Rank", "Date"].map(col => (
              <span key={col} className="text-[10px] text-slate-600 uppercase tracking-widest font-bold">
                {col}
              </span>
            ))}
          </div>
              ...
              {processedHistory.map((entry, i) => {
                ...
                return (
                  <div
                    ...
                    className={`grid grid-cols-[3rem_1fr_1fr_1fr_6rem] gap-4 px-6 py-4 hover:bg-red-950/10 transition-all duration-300 ${
                      isRevealed ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"
                    }`}
                  >
                    <span className="text-xs font-bold text-slate-600 self-center">#{entry.roundId}</span>
                    ...
                    <span className="text-[11px] text-slate-600 self-center">{formatTimestamp(entry.endTime)}</span>
                  </div>
                );
              })}
            </div>
          ...
          {processedHistory.length > 0 && (
            <div className="px-6 py-3 bg-red-950/10 border-t border-red-900/20 flex justify-between items-center">
        ...
        {/* ── Rank legend ────────────────────────────────────────────────────── */}
        <div className="rounded-2xl border border-red-900/20 bg-black/30 p-6 space-y-4 backdrop-blur-sm">
          <p className="text-[10px] text-slate-600 uppercase tracking-widest font-bold">Tier Classification</p>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              <div key={rank} className="space-y-1">
                ...
                <p className="text-[11px] text-slate-700">{range}</p>
              </div>
            ))}
          </div>
        </div>
